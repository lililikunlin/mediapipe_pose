...
VideoSource="樹式_BCDvv4UWL8s.mp4" # ← 改為影片來源
...
KNEE_ANGLE_L2 = (130, 160)     # Level 2：更彎
KNEE_ANGLE_L34 = (85, 130)     # Level 3/4：幾乎成直角到更彎
...
def classify_pose(person, img_w=None, img_h=None):
    ...
    樹式四級（自動左右腿）：
    - 共同條件：23-24 水平
    - Level 1：同側膝微彎；抬腳跟 ≈ 另一側踝（相碰）
    - Level 2：同側膝更彎；抬腳跟在「另一側膝」下方一點點
    - Level 3：同側膝更彎（幾乎平）；抬腳跟靠近「另一側髖」
